...
function logout() {
  ...
  // Changed from login.html to simulator.html
...
function doLogin() {
  ...
  if (users[email] && users[email].pass === btoa(pass)) {
    ...
    // Changed from index.html to simulator.html (self-redirect or app refresh)
    setTimeout(() => { window.location.href = 'simulator.html'; }, 1200);
  } else {
